...
def shortest_path(source, target):
    ...
    while True:
        ...
        nodeNeighbors = neighbors_for_person(node.state)
        for neighbor in nodeNeighbors:
            ...
            if neighbor[0] not in exploredMovies:
                child = Node(state=neighbor[1], action=neighbor[0], parent=node)
                frontier.add(child)

        for movieId in people[node.state]["movies"]:
            exploredMovies.add(movieId)
